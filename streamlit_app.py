...
# --- 2. Connections & Data Setup ---
conn = st.connection("gsheets", type=GSheetsConnection)

@st.cache_resource
def get_mine_list():
    """Automatically fetches the list of all worksheets in the spreadsheet."""
    try:
        # Use the same credentials as defined in GSheetsConnection
        ...
        # Exclude internal/technical sheets
        exclude = ["Dashboard", "Template"] 
        return [name for name in all_worksheets if name not in exclude]
    except Exception as e:
        st.error(f"Error fetching mine list: {e}")
        return []

# Dynamic list of mines
MINE_LIST = get_mine_list()
...
if selected_mine:
    ...
    st.warning("No mines available for analysis. Please check your Google Sheets connection.")